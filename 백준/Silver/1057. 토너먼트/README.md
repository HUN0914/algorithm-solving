...
서로의 위치가 동일해질 경우, 그때의 count 값을 구하면 되는 문제이다.
(만약 count가 max 범위를 초과하는 경우 -1을 출력하고 break)

그리고, 각각의 위치는 자신의 숫자에 +1을 한 후 /2를 한 값이다

(1과 2의 대결 후 이긴 사람의 위치는 만약 1이 주인공이면 (1+[1])/2 =1),
2가 주인공이라면 (2+[1]/2=1)이 되므로 1과 2의 대결에서 이기는 자의 위치는 1이다.)

생각을 좀 더 논리적으로 해보도록 노력하자.
