...
만약 CASE가 32라면 [아무리 제일 큰 경우]를 고려하더라도

중복되는거 2개는 존재

근데, 33이 되는 경우에는 최소 3개가 중복이 된다 (-> 값이 0이 된다는 의미)

그렇기에, 32 초과일떄부턴 CASE=0으로 계산하여 시간 복잡도를 줄인다. (이게 바로 *비둘기집의 원리*이다.)
