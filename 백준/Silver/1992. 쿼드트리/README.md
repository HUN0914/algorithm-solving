...
return의 조건을 0,1로 생각하지말고 '어떠한 [조건] 에서 멈출지를 생각을 하자.'

-> 0이면, 어떨 때 0이 되는지, 왜 0이 되는 상황이 나오는지를 생각

위 문제와 같은 경우에는 탐색이 끝났을 때 return을 해줘야 한다. 라는 조건을 생각하고 간다.

또한, 항상 return문을 따로 빼주는건 아니다 (이 문제에서 첨 알았다ㅠ)
