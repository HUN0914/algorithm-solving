...
C++에서 SORT 알고리즘의 compare(임의 정렬) 사용법

sort에서 들어간 두 매개변수의 값 자체만 비교하는 것이 아니다.

매개변수의 모든 것을 비교하여 return으로 넘겨줄 수 있다.
...
예를 들어,
 ...
 와 같이 size를 비교하여 return 값으로 넘겨줄 수도 있고,

 그냥 사전순 그자체로 비교한 값을 넘겨줄 수도 있다.
...
문자열에 들어간 이상, 숫자인 값은 숫자 0~9가 아니라 ASCII 코드의 

*Number 48~값으로 인식이 된다.*
