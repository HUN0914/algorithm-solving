...
-1의 조건을 굳이굳이 생각하려 하지말고
0일때에만 -1 출력하도록 하면 좀 더 접근이 쉬워진다.
+) 방문 정보가 MAIN이 아니라면 굳이 visited 배열을 쓰려고 하지마라.

몇일만에 익는지 궁금한것이기 때문에 미리 체킹된 값에도 +를 해주면서 
최종적으로 다 익으려면 얼마나의 시간이 걸리는지를 체크하면 되는 문제

문제의 핵심은

익음의 여부 뿐만이 아닌 익을 때의 주변이 익은지 얼마나 탐색하는 것이다.
