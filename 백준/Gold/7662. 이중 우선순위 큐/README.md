...
Multiset : 키 값이 중복으로 가질 수 있는 집합 자료구조 (값을 넣을 때마다 정렬된 어떤 배열 형식을 원한다면 쓸 때 좋을듯)

오늘의 배운점 : 우선순위 큐라고 무조건 그렇게 풀지 않기.

iterator같은 경우 auto iter;로 선언
auto iter= ~~.end(); 이렇게 선언하면 된다.

출력할때는 *iter로 출력하면 됨.

iter도 ++, -- 연산 다 가능하다.
선언 방법 :  multiset<int> ms;
